...
class TetrodeGridFaust : public FaustImpl {
 ...
 public:
	...
	virtual TetrodeGridFaust* clone() {
		return new TetrodeGridFaust();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow9 = (iSlow8 ? 0.0f : std::exp((0.0f - (fConst2 / (iSlow8 ? 1.0f : fSlow7)))));
		...
		float fSlow16 = (1.0f - (1.0f / ((fConst0 * (fSlow13 * float(fEntry7))) + 1.0f)));
		for (int i = 0; (i < count); i = (i + 1)) {
			float fTemp0 = float(input0[i]);
			float fTemp1 = (fTemp0 - fSlow4);
			fVec0[0] = fTemp1;
			float fTempFTZ0 = ((fSlow3 * fVec0[1]) - (fSlow5 * ((fSlow6 * fRec0[1]) - (fSlow1 * fTemp1))));
			fRec0[0] = ((*reinterpret_cast<int*>(&fTempFTZ0) & 2139095040) ? fTempFTZ0 : 0.0f);
			float fTempFTZ1 = ((fRec1[1] * fSlow9) + ((fRec0[0] - fSlow10) * fSlow11));
			fRec1[0] = ((*reinterpret_cast<int*>(&fTempFTZ1) & 2139095040) ? fTempFTZ1 : 0.0f);
			float fTempFTZ2 = ((fSlow14 * (std::max<float>(0.0f, (fSlow12 - fRec2[1])) * std::max<float>(0.0f, (std::max<float>(0.0f, ((fRec0[0] - fRec1[0]) - fSlow15)) - fRec2[1])))) + (fSlow16 * fRec2[1]));
