...
impl<'a> Window<'a> {
    ...
    /// now, ahead of its regular schedule. `None` when the platform has no
    /// such mechanism or the window runs without a run loop.
    #[cfg(target_os = "macos")]
    pub fn frame_waker(&self) -> Option<crate::macos::FrameWaker> {
